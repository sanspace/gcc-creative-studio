...
class Storyboard(Base):
    ...
    bg_music_description: Mapped[str | None] = mapped_column(String, nullable=True)
    bg_music_asset_id: Mapped[int | None] = mapped_column(ForeignKey("media_items.id"), nullable=True)

    # project: Mapped["Project"] = relationship(back_populates="storyboard")
    scenes: Mapped[list["Scene"]] = relationship(back_populates="storyboard", cascade="all, delete-orphan")
    timeline: Mapped["Timeline"] = relationship(back_populates="storyboard", uselist=False, cascade="all, delete-orphan")
...
class Scene(Base):
    ...
    # First Frame Prompt
    first_frame_description: Mapped[str | None] = mapped_column(String, nullable=True)
    first_frame_media_item_id: Mapped[int | None] = mapped_column(ForeignKey("media_items.id"), nullable=True)
    first_frame_source_asset_id: Mapped[int | None] = mapped_column(ForeignKey("source_assets.id"), nullable=True)
    first_frame_generated_url: Mapped[str | None] = mapped_column(String, nullable=True)
    ...
    video_media_item_id: Mapped[int | None] = mapped_column(ForeignKey("media_items.id"), nullable=True)
    video_source_asset_id: Mapped[int | None] = mapped_column(ForeignKey("source_assets.id"), nullable=True)
    video_generated_url: Mapped[str | None] = mapped_column(String, nullable=True)
    ...
    voiceover_description: Mapped[str | None] = mapped_column(String, nullable=True)
    voiceover_media_item_id: Mapped[int | None] = mapped_column(ForeignKey("media_items.id"), nullable=True)
    voiceover_source_asset_id: Mapped[int | None] = mapped_column(ForeignKey("source_assets.id"), nullable=True)
    ...
    audio_ambient_description: Mapped[str | None] = mapped_column(String, nullable=True)
    audio_sfx_description: Mapped[str | None] = mapped_column(String, nullable=True)
...
class Timeline(Base):
    ...
    video_clips: Mapped[list["VideoClip"]] = relationship(back_populates="timeline", cascade="all, delete-orphan")
    audio_clips: Mapped[list["AudioClip"]] = relationship(back_populates="timeline", cascade="all, delete-orphan")

class VideoClip(Base):
    __tablename__ = "video_clips"

    id: Mapped[int] = mapped_column(primary_key=True, autoincrement=True)
    timeline_id: Mapped[int] = mapped_column(ForeignKey("timelines.id"), nullable=False)
    media_item_id: Mapped[int | None] = mapped_column(ForeignKey("media_items.id"), nullable=True)
    source_asset_id: Mapped[int | None] = mapped_column(ForeignKey("source_assets.id"), nullable=True)
