...
def test_upload_source_asset_success(client, mock_service, mock_workspace_auth, mock_user):
    # Setup
    mock_response = SourceAssetResponseDto(
        id=10, workspace_id=1, user_id=1, gcs_uri="gs://b/1", original_filename="test.png",
        mime_type=MimeTypeEnum.IMAGE_PNG, aspect_ratio="1:1", file_hash="h",
        scope=AssetScopeEnum.PRIVATE, asset_type=AssetTypeEnum.GENERIC_IMAGE,
        presigned_url="https://signed.url", presigned_original_url="", presigned_thumbnail_url="", user_email="test@example.com"
    )
...
def test_list_source_assets_regular_user(client, mock_service, mock_user):
    ...
    mock_response = PaginationResponseDto[SourceAssetResponseDto](
        count=0, page=1, page_size=10, total_pages=0, data=[]
    )
...
async def test_list_source_assets_admin_search_self(client, mock_service, mock_user):
    from src.common.dto.pagination_response_dto import PaginationResponseDto
    mock_user.roles = ["admin"]
    mock_service.list_assets_for_user.return_value = PaginationResponseDto(count=0, data=[], page=1, page_size=10, total_pages=0)
...
async def test_list_source_assets_admin_search_other(client, mock_service, mock_user):
    ...
    with patch("src.source_assets.source_asset_controller.asyncio.to_thread") as mock_to_thread:
...
@pytest.mark.anyio
async def test_list_source_assets_admin_search_other_not_found(client, mock_service, mock_user):
    mock_user.roles = ["admin"]
    with patch("src.source_assets.source_asset_controller.asyncio.to_thread") as mock_to_thread:
