...
async def test_get_and_validate_aspect_ratio_image_deduced(service):
    ...
    # 160x90 is 16:9
    ratio = await service._get_and_validate_aspect_ratio(contents=contents, is_video=False)
...
async def test_get_and_validate_aspect_ratio_image_provided_valid(service):
    ...
    ratio = await service._get_and_validate_aspect_ratio(
        contents=contents, is_video=False, provided_aspect_ratio="9:16"
    )
...
async def test_get_and_validate_aspect_ratio_image_provided_invalid(service):
    ...
    with pytest.raises(HTTPException) as exc_info:
        await service._get_and_validate_aspect_ratio(
            contents=contents, is_video=False, provided_aspect_ratio="invalid_ratio"
        )
...
async def test_upload_asset_success_image(service, mock_dependencies, sample_user):
    ...
    mock_dependencies["iam_signer"].generate_presigned_url.return_value = "https://signed.url"
    
    # Setup create returns saved object
    saved_asset = SourceAssetModel(id=10, workspace_id=1, user_id=1, gcs_uri="gs://b", original_filename="a", file_hash="h", mime_type=MimeTypeEnum.IMAGE_PNG)
...
async def test_upload_asset_duplicate(service, mock_dependencies, sample_user):
    ...
    existing = SourceAssetModel(id=5, workspace_id=1, user_id=1, gcs_uri="gs://b", original_filename="a", file_hash="h", mime_type=MimeTypeEnum.IMAGE_PNG)
...
async def test_delete_asset(service, mock_dependencies):
    # Setup
    asset = SourceAssetModel(id=1, workspace_id=1, user_id=1, gcs_uri="gs://b", original_filename="a", file_hash="h", mime_type=MimeTypeEnum.IMAGE_PNG)
...
async def test_upload_asset_video(mock_generate_thumbnail, mock_get_dimensions, service, mock_dependencies, sample_user):
    # Setup Mocks
    ...
    mock_dependencies["gcs_service"].upload_file_to_gcs.return_value = "gs://bucket/vid.mp4"
    mock_dependencies["iam_signer"].generate_presigned_url.return_value = "https://signed.url"
    
    saved_asset = SourceAssetModel(id=20, workspace_id=1, user_id=1, gcs_uri="gs://b", original_filename="v", file_hash="h2", mime_type=MimeTypeEnum.VIDEO_MP4)
    mock_dependencies["repo"].create.return_value = saved_asset
...
async def test_upload_asset_with_upscale(service, mock_dependencies, sample_user):
    ...
    saved_asset = SourceAssetModel(id=30, workspace_id=1, user_id=1, gcs_uri="gs://b", original_filename="u", file_hash="h3", mime_type=MimeTypeEnum.IMAGE_PNG)
    mock_dependencies["repo"].create.return_value = saved_asset
...
async def test_get_all_vto_assets(service, mock_dependencies, sample_user):
    ...
    mock_asset1 = SourceAssetModel(id=1, workspace_id=1, user_id=1, gcs_uri="gs://b/1", original_filename="m", file_hash="h1", mime_type=MimeTypeEnum.IMAGE_PNG, asset_type=AssetTypeEnum.VTO_PERSON_MALE)
    mock_asset2 = SourceAssetModel(id=2, workspace_id=1, user_id=1, gcs_uri="gs://b/2", original_filename="f", file_hash="h2", mime_type=MimeTypeEnum.IMAGE_PNG, asset_type=AssetTypeEnum.VTO_PERSON_FEMALE)
    
    mock_dependencies["repo"].find_system_and_private_assets_by_types.return_value = [mock_asset1, mock_asset2]
    mock_dependencies["iam_signer"].generate_presigned_url.return_value = "https://signed.url"
    ...
    mock_dependencies["repo"].find_system_and_private_assets_by_types.assert_called_once()
...
@pytest.mark.anyio
async def test_get_asset_by_id_authorized_owner(service, mock_dependencies, sample_user):
    asset = SourceAssetModel(id=40, workspace_id=1, user_id=1, gcs_uri="gs://b", original_filename="a", file_hash="h4", scope=AssetScopeEnum.PRIVATE, mime_type=MimeTypeEnum.IMAGE_PNG)
    mock_dependencies["repo"].get_by_id.return_value = asset
...
async def test_create_from_gcs_uri_success(service, mock_dependencies, sample_user):
    ...
    mock_dependencies["gcs_service"].download_bytes_from_gcs.return_value = get_dummy_image_bytes()
    mock_dependencies["repo"].find_by_hash.return_value = None
    mock_dependencies["gcs_service"].store_to_gcs.return_value = "gs://bucket/out.png"
    mock_dependencies["iam_signer"].generate_presigned_url.return_value = "https://signed.url"

    saved_asset = SourceAssetModel(id=50, workspace_id=1, user_id=1, gcs_uri="gs://b", original_filename="a", file_hash="h5", mime_type=MimeTypeEnum.IMAGE_PNG)
    mock_dependencies["repo"].create.return_value = saved_asset
    ...
    mock_dependencies["gcs_service"].download_bytes_from_gcs.assert_called_once_with("gs://input/in.png")
    mock_dependencies["gcs_service"].store_to_gcs.assert_called_once()
    mock_dependencies["repo"].create.assert_called_once()
...
@pytest.mark.anyio
@patch("src.source_assets.source_asset_service.generate_thumbnail")
async def test_create_from_gcs_uri_video(mock_thumb, service, mock_dependencies, sample_user):
    # Mock download_bytes_from_gcs returning dummy video bytes
    mock_dependencies["gcs_service"].download_bytes_from_gcs.return_value = b"dummy_video_bytes"
    mock_dependencies["repo"].find_by_hash.return_value = None
    mock_dependencies["gcs_service"].upload_file_to_gcs.return_value = "gs://bucket/out.mp4"
    mock_dependencies["iam_signer"].generate_presigned_url.return_value = "https://signed.url"
    
    mock_thumb.return_value = "/tmp/thumbnails/thumb.png"

    saved_asset = SourceAssetModel(id=55, workspace_id=1, user_id=1, gcs_uri="gs://bucket/out.mp4", original_filename="a.mp4", file_hash="h55", mime_type=MimeTypeEnum.VIDEO_MP4)
    mock_dependencies["repo"].create.return_value = saved_asset
    ...
    mock_dependencies["gcs_service"].download_bytes_from_gcs.assert_called_once_with("gs://input/in.mp4")
...
async def test_upload_asset_non_png(service, mock_dependencies, sample_user):
    ...
    saved_asset = SourceAssetModel(id=60, workspace_id=1, user_id=1, gcs_uri="gs://b", original_filename="a", file_hash="h6", mime_type=MimeTypeEnum.IMAGE_PNG)
...
async def test_upload_asset_audio(service, mock_dependencies, sample_user):
    ...
    saved_asset = SourceAssetModel(id=70, workspace_id=1, user_id=1, gcs_uri="gs://b/audio.mp3", original_filename="s.mp3", file_hash="h7", mime_type=MimeTypeEnum.AUDIO_MPEG)
...
async def test_upload_asset_upscale_failure(service, mock_dependencies, sample_user):
    mock_dependencies["repo"].find_by_hash.return_value = None
    mock_dependencies["gcs_service"].store_to_gcs.return_value = "gs://bucket/asset.png"
    mock_dependencies["imagen_service"].upscale_image.side_effect = Exception("Upscale failed")
    mock_dependencies["iam_signer"].generate_presigned_url.return_value = "https://signed.url"

    saved_asset = SourceAssetModel(id=80, workspace_id=1, user_id=1, gcs_uri="gs://b", original_filename="a", file_hash="h8", mime_type=MimeTypeEnum.IMAGE_PNG)
