...
from src.workspaces.schema.workspace_model import WorkspaceModel, WorkspaceMember, WorkspaceRoleEnum
...
class TestCreateWorkspace:
    ...
    @pytest.mark.anyio
    async def test_create_workspace_success(self, workspace_service, mock_workspace_repo, mock_user):
        create_dto = CreateWorkspaceDto(name="New Workspace")
        
        # Mock repo to return the created workspace
        mock_workspace = WorkspaceModel(id=1, name="New Workspace", owner_id=mock_user.id)
...
class TestInviteUserToWorkspace:
    ...
    @pytest.mark.anyio
    async def test_workspace_not_found(self, workspace_service, mock_workspace_repo, mock_user):
        mock_workspace_repo.get_by_id.return_value = None
        invite_dto = InviteUserDto(email="guest@example.com", role=WorkspaceRoleEnum.VIEWER)
    ...
    @pytest.mark.anyio
    async def test_forbidden_not_owner_or_admin(self, workspace_service, mock_workspace_repo, mock_user):
    ...
    @pytest.mark.anyio
    async def test_user_not_found(self, workspace_service, mock_workspace_repo, mock_user_repo, mock_user):
        ...
        invite_dto = InviteUserDto(email="unknown@example.com", role=WorkspaceRoleEnum.VIEWER)

        result = await workspace_service.invite_user_to_workspace(1, invite_dto, mock_user)

        assert result is None
        mock_user_repo.get_by_email.assert_called_once_with("unknown@example.com")

    @pytest.mark.anyio
    async def test_success_invite(self, workspace_service, mock_workspace_repo, mock_user_repo, mock_email_service, mock_user):
        workspace = WorkspaceModel(id=1, name="Test", owner_id=mock_user.id)
        mock_workspace_repo.get_by_id.return_value = workspace
        
        # Mock invited user
        from src.users.user_model import UserModel
        invited_user = UserModel(id=2, email="guest@example.com", roles=[], name="Guest")
...
class TestListWorkspacesForUser:
    ...
    @pytest.mark.anyio
    async def test_list_workspaces_combine_lists(self, workspace_service, mock_workspace_repo, mock_user):
        w1 = WorkspaceModel(id=1, name="Private 1", owner_id=mock_user.id)
        w2 = WorkspaceModel(id=2, name="Public 1", owner_id=99)  # Public, owned by someone else
