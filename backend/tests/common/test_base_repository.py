...
async def test_create_success():
    ...
    input_model = SourceAssetModel(
        workspace_id=1, user_id=1, gcs_uri="gs://b/1",
        original_filename="1", file_hash="h1", mime_type="image/png",
        aspect_ratio="1:1", scope="private", asset_type="generic_image"
    )
...
async def test_restore_success():
    mock_db = AsyncMock()
    mock_asset = get_dummy_source_asset(id=50, deleted_at=datetime.datetime.now(datetime.timezone.utc))
    mock_result = MagicMock()
    mock_result.scalar_one_or_none.return_value = mock_asset
    mock_db.execute.return_value = mock_result
