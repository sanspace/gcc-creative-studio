...
def test_upload_file_to_gcs_success(gcs_service):
    ...
    with patch("src.common.storage_service.pathlib.Path") as mock_path:
        ...
        mock_blob.upload_from_filename.assert_called_once_with("/tmp/local.txt", content_type="text/plain")
...
def test_upload_file_to_gcs_api_error(gcs_service):
    mock_blob = MagicMock()
    mock_blob.upload_from_filename.side_effect = exceptions.GoogleAPICallError("API error")
    gcs_service.bucket.blob.return_value = mock_blob
...
def test_store_to_gcs_bytes_success(gcs_service):
    ...
    mock_blob.upload_from_string.assert_called_once_with(b"bytes", content_type="text/plain")
...
def test_store_to_gcs_decode_success(gcs_service):
    ...
    res = gcs_service.store_to_gcs("folder", "file.txt", "text/plain", encoded, decode=True)
    assert res == "gs://test-bucket/folder/file.txt"
    mock_blob.upload_from_string.assert_called_once_with(b"hello", content_type="text/plain")
