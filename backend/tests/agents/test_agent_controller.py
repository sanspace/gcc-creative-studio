...
async def test_get_sessions_success(mock_async_client, client):
    ...
    mock_response.json.return_value = [{"id": "session_1", "appName": "creative_toolbox", "userId": "1"}]
    ...
    assert response.json() == [{"id": "session_1", "appName": "creative_toolbox", "userId": "1", "lastUpdateTime": None, "state": None, "events": None}]
...
async def test_poll_session_events_success(client, mock_db):
    ...
    dummy_event = AgentChatEvent(id=1, user_id="1", session_id="s1", payload={"raw": "data: event1"})
