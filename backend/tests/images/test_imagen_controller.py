...
from src.images.schema.imagen_result_model import ImageGenerationResult, CustomImagenResult
...
def test_generate_images_success(client, mock_service, mock_workspace_auth):
    mock_response = MediaItemResponse(
        ...
        mime_type=MimeTypeEnum.IMAGE_PNG, status=JobStatusEnum.PROCESSING,
        model=GenerationModelEnum.IMAGEN_3_001, gcs_uris=[], presigned_urls=[],
        presigned_thumbnail_urls=[], aspect_ratio="1:1"
    )
...
def test_upload_upscale_success(client, mock_service, mock_workspace_auth):
    mock_response = MediaItemResponse(
        id=333, workspace_id=1, user_id=1, user_email="test@example.com",
        mime_type=MimeTypeEnum.IMAGE_PNG, status=JobStatusEnum.PROCESSING,
        model=GenerationModelEnum.IMAGEN_4_UPSCALE_PREVIEW, gcs_uris=[], presigned_urls=[],
        aspect_ratio="1:1"
    )
...
def test_upscale_image_api_success(client, mock_service):
    mock_result = ImageGenerationResult(
        enhanced_prompt="",
        rai_filtered_reason="",
        image=CustomImagenResult(gcs_uri="gs://b/u.png", encoded_image="", mime_type=MimeTypeEnum.IMAGE_PNG, presigned_url="")
    )
    mock_service.upscale_image.return_value = mock_result

    payload = {
        "user_image": "gs://b/i.png",
        "upscale_factor": "x2"
    }
...
def test_generate_images_http_exception(client, mock_service):
    ...
    mock_service.start_image_generation_job.side_effect = HTTPException(status_code=400, detail="Custom Bad Request")
...
def test_generate_images_vto_http_exception(client, mock_service):
    from fastapi import HTTPException
    mock_service.start_vto_generation_job.side_effect = HTTPException(status_code=400, detail="VTO Failed")
...
def test_upscale_image_api_http_exception(client, mock_service):
    from fastapi import HTTPException
    mock_service.upscale_image.side_effect = HTTPException(status_code=400, detail="Upscale Failed")
...
def test_generate_images_vto_value_error(client, mock_service):
    ...
    payload = {"workspace_id": 1, "person_image": {"source_asset_id": 101}, "top_image": {"source_asset_id": 102}}
    response = client.post("/api/images/generate-images-for-vto", json=payload)
    assert response.status_code == 400
