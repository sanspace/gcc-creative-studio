...
@pytest.fixture
def mock_admin():
    return UserModel(id=1, email="admin@example.com", name="Admin", roles=[UserRoleEnum.ADMIN])

@pytest.fixture
def mock_user():
    return UserModel(id=2, email="user@example.com", name="User", roles=[UserRoleEnum.USER])
...
def test_search_gallery_items_admin_with_workspace(mock_admin, mock_service, mock_workspace_auth):
    ...
    mock_service.get_paginated_gallery.return_value = PaginationResponseDto(count=1, data=[], page=1, page_size=10, total_pages=1)
