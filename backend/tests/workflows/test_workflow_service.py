...
from src.workflows.schema.workflow_run_model import WorkflowRunModel, WorkflowRunStatusEnum
...
class TestCreateWorkflow:
    ...
    async def test_create_workflow_success(self, mock_client_class, workflow_service, mock_workflow_repo, sample_workflow_create_dto, sample_user):
        # Mock GCP Client
        ...
        mock_workflow_repo.create.return_value = WorkflowModel(
            id="id-123", user_id=1, name="Test", steps=sample_workflow_create_dto.steps
        )

        result = await workflow_service.create_workflow(sample_workflow_create_dto, sample_user)
    ...
    @pytest.mark.anyio
    @patch("src.workflows.workflow_service.workflows_v1.WorkflowsClient")
    async def test_create_workflow_gcp_failure_rollback(self, mock_client_class, workflow_service, mock_workflow_repo, sample_workflow_create_dto, sample_user):
        mock_client = MagicMock()
        mock_client_class.return_value = mock_client
        mock_client.create_workflow.side_effect = Exception("GCP Error")

        created_model = WorkflowModel(id="id-123", user_id=1, name="Test", steps=sample_workflow_create_dto.steps)
        mock_workflow_repo.create.return_value = created_model

        with pytest.raises(Exception) as exc_info:
            await workflow_service.create_workflow(sample_workflow_create_dto, sample_user)
...
class TestExecuteWorkflow:
    ...
    async def test_execute_workflow_success(self, mock_exec_client_class, workflow_service, mock_workflow_repo, mock_run_repo, sample_workflow_model, sample_user):
        # Setup
        ...
        mock_exec_client.create_workflow_execution = AsyncMock(return_value=mock_response) 
        ...
        # Execute
        exec_id = await workflow_service.execute_workflow(workflow_id="id-123", args=args, user=sample_user)
...
class TestGetExecutionDetails:
    ...
    async def test_get_execution_details_success(self, mock_auth_session_class, mock_auth_default, mock_exec_client_class, workflow_service, mock_run_repo, sample_workflow_model):
        ...
        mock_response.json.return_value = {"stepEntries": [{"step": "step_1", "state": "STATE_SUCCEEDED"}]}
        ...
        # Execute
        details = await workflow_service.get_execution_details(workflow_id="id-123", execution_id="e-123")
...
class TestBatchExecuteWorkflow:
    ...
    async def test_batch_execute_success(self, workflow_service, sample_user):
        from src.workflows.dto.batch_execution_dto import BatchExecutionRequestDto, BatchExecutionItemDto
        ...
        response = await workflow_service.batch_execute_workflow(
            workflow_id="id-123", batch_dto=batch_dto, user=sample_user
        )
    ...
    @pytest.mark.anyio
    async def test_batch_execute_gcs_ingestion_success(self, workflow_service, sample_user):
        from src.workflows.dto.batch_execution_dto import BatchExecutionRequestDto, BatchExecutionItemDto
        ...
        mock_asset.id = 100
        workflow_service.source_asset_service.create_from_gcs_uri = AsyncMock(return_value=mock_asset)

        # Build DTO with GCS URI
        batch_dto = BatchExecutionRequestDto(
            items=[
                BatchExecutionItemDto(row_index=0, args={"image": "gs://bucket/img.jpg", "workspace_id": "1"})
            ]
        )
    ...
    @pytest.mark.anyio
    async def test_batch_execute_gcs_ingestion_no_workspace_id_failure(self, workflow_service, sample_user):
        from src.workflows.dto.batch_execution_dto import BatchExecutionRequestDto, BatchExecutionItemDto
        ...
        batch_dto = BatchExecutionRequestDto(
            items=[
                BatchExecutionItemDto(row_index=0, args={"image": "gs://bucket/img.jpg"})
            ]
        )
...
class TestUpdateAndUpdateMethods:
    ...
    @pytest.mark.anyio
    @patch("src.workflows.workflow_service.workflows_v1.WorkflowsClient")
    async def test_update_workflow_success(self, mock_client_class, workflow_service, mock_workflow_repo, sample_workflow_create_dto, sample_user):
        mock_client = MagicMock()
        mock_client_class.return_value = mock_client
        mock_operation = MagicMock()
        mock_operation.result.return_value = MagicMock()
        ...
        updated_model = WorkflowModel(id="id-123", user_id=1, name="Updated", steps=sample_workflow_create_dto.steps)
        mock_workflow_repo.update.return_value = updated_model

        result = await workflow_service.update_workflow(workflow_id="id-123", workflow_dto=sample_workflow_create_dto, user=sample_user)
    ...
    @pytest.mark.anyio
    @patch("src.workflows.workflow_service.workflows_v1.WorkflowsClient")
    async def test_delete_by_id_success(self, mock_client_class, workflow_service, mock_workflow_repo):
        mock_client = MagicMock()
        mock_client_class.return_value = mock_client
        mock_operation = MagicMock()
        mock_operation.result.return_value = MagicMock()
