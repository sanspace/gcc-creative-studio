...
import {AgentChatService, SSECallbacks} from './services/agent-chat.service';
...
export class WorkbenchComponent implements OnInit, OnDestroy {
  ...
  sessions = signal<any[]>([]);
  topics = signal<{[key: string]: string}>({});
  ...
  dropdownOptions = computed<DropdownOption[]>(() => {
    ...
    return this.sessions().map(s => {
      ...
      const date = s.lastUpdateTime
        ...
        : '';
      const label = topic ? topic : date ? `${date} - Chat` : 'New Chat';
      return {
        value: s.id,
        label: label,
      };
  ...
  saveSessionTopic(sessionId: string, topic: string) {
    this.topics.update(t => {
      const newTopics = {...t, [sessionId]: topic};
      localStorage.setItem('izumi_topics', JSON.stringify(newTopics));
      return newTopics;
    });
  }

  getSessionTopic(sessionId: string): string {
    return this.topics()[sessionId] || '';
  }

  initializeAgentChat() {
    const savedTopics = JSON.parse(
      localStorage.getItem('izumi_topics') || '{}',
    );
  ...
  addWelcomeMessage() {
    const welcomeMessage = {
      sender: 'agent',
      text: "Hi! I'm Izumi, your GenMedia Marketing AI Coworker! I can help you create stunning creative brief campaigns, storyboard scripts, and scenes to generate a final GREAT video for your creative content or ads! How can I help you today?",
      timestamp: new Date(),
    };
  ...
  sendChatMessage(text: string) {
    ...
    if (this.chatMessages().length === 1) {
      this.agentChatService.generateTitle(text).subscribe({
        next: (response: any) => {
          this.saveSessionTopic(this.currentSessionId!, response.title);
        },
        error: err => {
